...
# class定義はmain.pyからshcemas.pyに移行する
...
# @app.get("/rooms") -> 元々の記載をここから作成したcrud.pyやschema.pyとつなげる
# async def read_rooms(rooms: Room):
#     return {"rooms": rooms}
...
# API部分については、schemaでUserのスキーマではなく、UserCreateにする
@app.post("/users", response_model=schemas.User)
